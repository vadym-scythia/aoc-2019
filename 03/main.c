...
#define ROWS 17000 // 17000
#define COLS 17000 // 17000
...
{   
    ...
    field[centerX][centerY] = 1;

    const int startValueForTheFirstWire = 1000000, startValueForTheSecondWire = 2000000;
    int isNext = 0, prevX = centerX, prevY = centerY, stepsCount1 = startValueForTheFirstWire, stepsCount2 = startValueForTheSecondWire, stepsMin = INT_MAX;
    while (*inputString != '\0')
    {
        int value = 0;
        ...
        if (*inputString == '/')
        {
            isNext = 1, prevX = centerX, prevY = centerY;
            inputString++;
        }

        char direction = *inputString;
        inputString++;

        if (*(inputString + 1) == ',' || *(inputString + 1) == '/' || *(inputString + 1) == '\0')
        {
            value = *inputString - '0';
            inputString++;
        }
        else if (*(inputString + 2) == ',' || *(inputString + 2) == '/' || *(inputString + 2) == '\0')
        {
            value = 10 * (*inputString - '0') + *(inputString + 1) - '0';
            inputString += 2;
        }
        else if (*(inputString + 3) == ',' || *(inputString + 3) == '/' || *(inputString + 3) == '\0')
        {
            value = 100 * (*inputString - '0') + 10 * (*(inputString + 1) - '0') + *(inputString + 2) - '0';
            inputString += 3;
        }
        ...
        case 'R':
            ...
            {
                int currentStepsCount1 = field[prevX][prevY + i] - startValueForTheFirstWire;

                if (isNext == 1) // here and in other switch cases code could be extracted to the separate function
                    if (currentStepsCount1 < startValueForTheFirstWire && currentStepsCount1 > 0)
                    {
                        stepsCount2++;
                        int currentStepsCount2 = stepsCount2 - startValueForTheSecondWire, sumPureStepsCount = currentStepsCount1 + currentStepsCount2;
                        
                        if (sumPureStepsCount < stepsMin)
                            stepsMin = sumPureStepsCount;

                        field[prevX][prevY + i] = 3;
                    }
                    else
                    {
                        stepsCount2++;
                        field[prevX][prevY + i] = stepsCount2;
                    }
                else
                {
                    stepsCount1++;
                    field[prevX][prevY + i] = stepsCount1;
                }
        ...
        case 'U':
            for (int i = 1; i <= value; i++)
            {
                int currentStepsCount1 = field[prevX + i][prevY] - startValueForTheFirstWire;
                ...
                if (isNext == 1)
                    if (currentStepsCount1 < startValueForTheFirstWire && currentStepsCount1 > 0)
                    {
                        ...
                        field[prevX + i][prevY] = 3;
                    }
                    else
                    {
                        stepsCount2++;
                        field[prevX + i][prevY] = stepsCount2;
                    }
                else
                {
                    stepsCount1++;
                    field[prevX + i][prevY] = stepsCount1;
                }
    ...
    for (int i = 0; i < ROWS; i++)
    {
        for (int j = 0; j < COLS; j++)
        {
            if (field[i][j] == 3)
            {
                int manhDistance = manhattanDistance(centerX, j, centerY, i);
    ...
    printf("result for the first part: %d\r\n", min);
    printf("result for the secondt part: %d", stepsMin);
